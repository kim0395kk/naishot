...
def parse_manual_md(md_content: str, filename: str = "") -> Dict:
    ...
    # (2) 적절한 헤더가 없으면 파일명 사용
    if not title:
        try:
            base_name = os.path.basename(filename)
            # 확장자 제거
            name_no_ext = os.path.splitext(base_name)[0]
            # 숫자+점+공백 패턴 제거 (예: "6-7-1. ", "1. ")
            name_clean = re.sub(r'^[\d\-\.]+\s*', '', name_no_ext)
            # 언더바(_)를 공백으로 변환
            title = name_clean.replace('_', ' ').strip()
        except Exception:
            title = "알 수 없음 (파일명 오류)"
...
{chr(10).join([f"- {m['event']}: {m['date']}" for m in data['milestones']])}
    ...
    # [Case B] 일반 업무 매뉴얼
        ...
        # 일반 매뉴얼은 1000자 단위 또는 논리적 단위로 자르는 것이 좋으나,
        # 현재는 파일 크기가 작으므로 전체를 하나의 큰 청크로 처리하되,
        # 필요 시 섹션별 분할 로직 추가 가능.
        
        chunks.append({
            "type": "manual_full",
            "complex_name": data['name'], # 호환성을 위해 complex_name 필드 유지하되 문서명 넣음
            "display_name": data['name'], # 화면 표시용 이름
            "text": data['raw_text'],
            "metadata": {
                "name": data['name'],
